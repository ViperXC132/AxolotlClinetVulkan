...
public class AxoGraphicsWidget extends GraphicsWidget {
	...
	@Override
	public void onPress() {
		MinecraftClient.getInstance().openScreen(new AxoGraphicsEditorScreen(MinecraftClient.getInstance().currentScreen, this.option));
	}
	...
	public static class AxoGraphicsEditorScreen extends GraphicsEditorScreen {
		...
		public void init() {
			...
			addDrawableChild(new VanillaButtonWidget(buttonX, buttonY + 25, 100, 20, new TranslatableText("graphics.copy_text"),
				btn -> client.keyboard.setClipboard(Base64.getEncoder().encodeToString(graphics.getPixelData()))));
			addDrawableChild(new VanillaButtonWidget(buttonX, buttonY + 50, 100, 20, new TranslatableText("graphics.paste_text"),
				btn -> {
					try {
